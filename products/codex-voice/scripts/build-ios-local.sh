...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "Codex Voice native builds must run directly on the operator's Mac." >&2
  exit 1
fi

if [ -n "$(git status --porcelain=v1)" ]; then
  echo "Commit or remove local changes before checking or building Codex Voice." >&2
  exit 1
fi

eas=(npx --yes eas-cli@22.2.0)

echo "==> Checking Codex Voice native-build contracts"
...
artifact_path="$build_root/CodexVoice.ipa"
...
echo "==> Building the signed Codex Voice IPA on this Mac"
...
echo "Codex Voice was built locally, submitted to TestFlight, and registered with Expo."
